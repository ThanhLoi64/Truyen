"use client";
import { GetAdminEditChapter, AdminEditChapter } from '@/app/api/user';
...
const EditChapterPage = () => {
  ...
  const fetchChapterDetails = async (postItemId) => {
    try {
      const chapterData = await GetAdminEditChapter(postItemId);
  ...
  const handleSubmit = async (e, saveDraft, savePublic) => {
    ...
    if (title.length < 3) {
      setSnackbarSeverity("error");
      setSnackbarMessage("Tiêu đề có ít nhất 3 ký tự");
      setOpenSnackbar(true);
      return;
    } 
    ...
    try {
      await AdminEditChapter(postItemId, chapterData);
      setError('');
